...
from git_checker import is_git_repo   # <-- new import
...
class FilePickerModal(ModalScreen):
    ...
    async def refresh_entries(self) -> None:
        ...
        for entry in self._entries():
            ...
            items.append(ListItem(Label(label, markup=False), name=str(entry)))
    ...
    async def on_list_view_selected(self, event: ListView.Selected) -> None:
        ...
        if selected_path.is_dir():
            ...
            # Do not dismiss on files – just show a hint in the status bar
            self.query_one("#file-picker-status", Label).update(
                f"{selected_path.name} is a file. Click a directory to enter it, or press 'o' to select the current directory."
            )
